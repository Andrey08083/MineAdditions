...
public class GuiHandler implements IGuiHandler {
	...
	public static final int GUI_BASEFURNACE = id++;
	
	@Override
	public Object getServerGuiElement(int ID, EntityPlayer player, World world, int x, int y, int z) {
		if(ID == GUI_BASEFURNACE) {
				System.out.println("[S] furnace != null");
				return new ContainerThermalCrusher(player.inventory, (ThermalCrusher)world.getTileEntity(new BlockPos(x, y, z)));
        }
        /*else if(ID == GUI_2) {
            return new Container2(player);
        }*/
		return null;
	}
